...
2. **생성층이 지나치게 보수적이다.** 근거를 쥐고도 모른다고 답하는 경우(h2-6)를
   줄여야 한다. "근거에 없으면 모른다"와 "근거를 끝까지 읽어라"의 균형 문제다.
3. **골든셋 12문항은 너무 적다.** 한 문항이 8%p 를 움직인다. 최소 30문항은 있어야
   홉수별 비교가 의미를 갖는다.
4. **한국어 요건 축의 마지막 한 칸.** `TOPIK 급수 ↔ 사회통합프로그램 단계` 대응을
   넣으면 한국어 쪽에서 물어도 비자 답이 나온다. 지금 그래프에서 `사회통합프로그램` 을
   거꾸로 걸으면 이미 체류자격 16개에 닿는다. 대응표만 붙이면 된다 — 다만 그건
   법무부 고시라 자주 바뀌므로, 넣을 때 최신본을 받아야 한다.
5. **멀티턴.** 이번엔 범위 밖으로 두었지만, 실제 사용자는 "저 E-9인데요"로 시작해
