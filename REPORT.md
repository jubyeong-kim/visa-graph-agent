...
실제로 그쪽으로 새고 있었다. 기대 경로는 12문항 합쳐 **21개**인데, 에이전트가 제시한
경로는 **63개**였다. 문항별로 보면 더 분명하다.
...
h1-2 는 10개를 퍼오고도 정작 답을 못 찾았다. 재현율 0% 라는 숫자 하나로는
"못 찾았다"만 알 수 있고, **"엉뚱한 걸 잔뜩 들고 왔다"는 사실은 안 보인다.**

이게 실제 실패로 이어졌다. **h2-6 은 경로 재현율 100% 인데 생성층 실패였다.**
필요한 근거를 다 쥐고도 잡음에 묻혀 "자료에 없다"고 답했다. 정밀도를 함께 봤다면
진작 보였을 문제다.
...
2. **근거를 너무 많이 퍼온다.** 기대 21개에 63개를 제시한다. 생성 모델이 관계없는
   삼중항을 읽느라 토큰을 쓰고, h2-6 처럼 잡음에 묻혀 답을 놓친다. 답변에 쓰인 경로만
   추려 `synthesize` 에 넘기고, 근거 예산을 관계별로 다시 재 봐야 한다.
   지금은 F1 로 **재기만** 했고 아직 **고치지는 않았다.**
