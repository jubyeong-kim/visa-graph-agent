...
def rule_triples(docs):
    ...
    # 귀화를 비자 그래프에 잇는 유일한 다리:
    #   「귀화허가 신청서」 "일반귀화 … 「민법」상 성년이며 영주자격(F-5)을 가지고 있는 사람"
    #
    # 이 문장을 찾기까지 한참 걸렸다. 「귀화」 안내문은 "대한민국에서 영주할 수 있는
    # 체류자격" 이라고만 쓰고, 국적법 전문에는 "영주자격"·"F-5" 가 **0회** 나온다.
    # 그래서 귀화가 통째로 섬이 되어 "영주권 받고 귀화하려면?" 에 답을 못 했다.
    # 코드에 F-5 라고 박아 넣을 수도 있었지만 그러면 출처를 댈 수 없는 사실이 된다.
    # 국적법 시행규칙 **별지서식**을 코퍼스에 넣어 근거를 만든 뒤 규칙으로 뽑는다.
    #
    # 서식이라 LLM 추출은 0개가 나온다(항목 라벨뿐이라 스키마에 걸리는 게 없다).
    NATUR = re.compile(r"(일반귀화|간이귀화|혼인귀화)[^[]{0,60}\[\s*\]\s*[^[]{0,40}?"
                       r"영주자격\(([A-H]-\d{1,2})\)")
    for title, body in docs.items():
        for m in NATUR.finditer(body):
    ...
    # 사용자가 가장 자주 묻는 축인데 LLM 추출에서 계속 빠졌다. 프롬프트에 적어
    # 봤지만 결과가 달라지지 않았다. 문장 모양이 일정하므로 규칙으로 내린다.
    #
    ...
    # 답했다. 없는 것을 모른다고 하는 것보다 **틀린 것을 단정적으로 말하는 쪽**이 나쁘다.
