...
|---|---|---|---|---|---|---|---|
| 1홉 | 3 | 2 | 2 | 67% | 12% | 0.21 | 4.0 |
| **2홉** | 6 | **4** | 3 | 69% | 15% | 0.24 | 10.8 |
| **3홉** | 2 | **2** | 0 | 100% | 17% | 0.29 | 18.0 |
| 전역 | 1 | 0 | 0 | — | — | — | — |
| 합계 | 12 | **8** | 5 | | | | |
...
**정밀도는 F1 을 도입한 뒤에 고쳤다.** 처음 재 보니 3홉 질문 하나에 근거 34개를
퍼와서 3개만 쓰고 있었다 — 재현율만 봤다면 "깊을수록 잘한다"로 읽었을 숫자다.
걷는 것과 보여 주는 것을 분리하고(질문 관련성으로 고르기, 한 노드당 상한, 총량 상한)
**근거를 절반으로 줄였는데 재현율과 정답률은 그대로**였다.
...
그래도 정밀도 17% 는 낮다. 기대 경로가 최소 사슬만 적어 주변 맥락은 분자에 안 들어가기
때문인데, 그걸 감안해도 아직 여유가 있다. 자세한 것은 [REPORT.md](REPORT.md).
