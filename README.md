...
있어서, BM25 가 그 문서만 집어 오면 이긴다. 그래프가 있어야 할 이유가 없는
질문이다. 실제로 2홉에서는 아직 진다.
...
자를 고정하자 한동안 결론이 뒤집혔다. 고정 전에는 `19:17 승` 이라고 적혀 있었고,
고정하니 **19:21 로 졌다.** 그 상태에서 실패 30건을 하나씩 읽어 세 가지를 고쳤고
...
- **조사 하나** — 기간 규칙이 `90일` 다음에 바로 `초과` 가 와야 잡았다. 안내문은
  ...
  옆에 있던 "15일 이내"(변경신고 기한)로 **틀린 답을 자신 있게** 했다.
