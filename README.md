...
**그리고 정밀도가 이 표의 아픈 부분이다.** 재현율은 홉이 깊어질수록 오르는데(67 → 75 →
100%) 정밀도는 8~9% 에 붙박이고 근거 수만 9 → 22 → 34 개로 불어난다. 3홉 질문 하나에
근거 34개를 퍼와서 그중 3개만 답에 쓴다. 재현율만 봤다면 "깊을수록 잘한다"고 읽었을
숫자다. **아직 재기만 했고 고치지는 않았다** — 자세한 것은 [REPORT.md](REPORT.md).
